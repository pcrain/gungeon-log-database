...
type
  LogDatabase* = ref object
    processedLogs : seq[ProcessedLog] # logs we've processed since launching
    ...
    gungeonVersionRX : CommonRegex # regex for detecting the version of gungeon running
    bepinexVersionRX : CommonRegex # regex for detecting the version of bepinex running
    platformRX       : CommonRegex # regex for detecting the OS running
    timestampRX      : CommonRegex # regex for detecting when a log was created
    patcherRX        : CommonRegex # regex for detecting whether we've found the MTG API patcher
    interfaceRX      : CommonRegex # regex for detecting the platform interface in the log
    modRX            : CommonRegex # regex for detecting a mod
    exceptionRX      : CommonRegex # regex for detecting an exception
    exceptionAltRX   : CommonRegex # alternate regex for detecting an exception
    oldMtgRX         : CommonRegex # regex for detecting MTG Classic / ETGMOD
...
func ensureSetup(db : LogDatabase) : void =
  ...
  db.gungeonVersionRX = creCompile(r"\[Info\s*:\s*Unity Log\] Version: ([0-9h\.]+)")
  db.bepinexVersionRX = creCompile(r"\[Message\s*:\s*BepInEx\] BepInEx ([0-9\.]+) - .*")
  db.platformRX       = creCompile(r"\[Info\s*:\s*BepInEx\] System platform: (.*)\s*")
  db.timestampRX      = creCompile(r"\[Info\s*:\s*Unity Log\] Now: (.*)\s*")
  db.interfaceRX      = creCompile(r".*Starting (.*) platform interface.\s*")
  db.patcherRX        = creCompile(r"MTGAPIPatcher.mm.dll")
  db.modRX            = creCompile(r"\[Info\s*:\s*BepInEx\] Loading \[(.*) ([0-9]+\.[0-9]+(?:\.[0-9]+)?)\]")
  db.exceptionRX      = creCompile(r"\[Error\s*:\s*Unity Log\] ([^\s]*Exception)\s*:\s*(.*)")
  db.exceptionAltRX   = creCompile(r"\[Message\s*:\s*ETG Console\] An error occured when doing ([^\s]+):\s*([^\s]+Exception)\s*: (.*)")
  db.oldMtgRX         = creCompile(r"ETGMOD INIT")
...
func processLogLines*(db : LogDatabase, lines : seq[string]) : void =
  ...
  while not li.done():
    ...
    # scan for basic info
    if plog.bepinexVersion.len == 0 and line.creMatch(db.bepinexVersionRX, matches):
    ...
    if plog.timestamp.len == 0 and line.creMatch(db.timestampRX, matches):
      ...
      continue
    if plog.platformInterface.len == 0 and line.creMatch(db.interfaceRX, matches):
      plog.platformInterface = matches[0]
      continue
    if (not plog.foundMTGAPIPatcher) and (line.creFind(db.patcherRX)):
      plog.foundMTGAPIPatcher = true
      continue
    if (line.creFind(db.oldMtgRX)):
    ...
    if line.creMatch(db.exceptionAltRX, matches):
      db.processException(plog = plog, li = li.back(), name = matches[1], message = matches[2])
      continue
    if line.creMatch(db.exceptionRX, matches):
